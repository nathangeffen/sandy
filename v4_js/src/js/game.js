...
const REGEX_PLY_COUNT = new RegExp(REGEX_PLY_LAST_POINTS.source + "-" + REGEX_PLY_TILL_END.source);
const REGEX_REPEAT_LAST_MOVE = /(?<noRepeatLastMove>([tf]))/;
// const REGEX_MOVES = /(?<moves>((\d+)-(\d+))*)/;
const REGEX_POSITION = new RegExp(REGEX_VERSION.source +
    "#" +
...
export const DEFAULT_POSITION_STRING = DEFAULT_VERSION +
    "#" +
    DEFAULT_SIZE +
    "#" +
    DEFAULT_PIECES +
...
(function (GameOverReason) {
    GameOverReason[GameOverReason["None"] = 0] = "None";
    GameOverReason[GameOverReason["Agreement"] = 1] = "Agreement";
    GameOverReason[GameOverReason["Resignation"] = 2] = "Resignation";
    GameOverReason[GameOverReason["NoMoves"] = 3] = "NoMoves";
    GameOverReason[GameOverReason["PlyWithoutPoints"] = 4] = "PlyWithoutPoints";
    GameOverReason[GameOverReason["PointsScored"] = 5] = "PointsScored";
...
export const getMoves = function (game) {
    const removeRepeatedMove = function (moves) {
...
const checkGameStatus = function (game, side) {
    ...
    const getResult = function () {
        ...
        moves = [];
        return (score[0] === score[1]) ? GameStatus.Tie : ((score[0] > score[1]) ? GameStatus.South : GameStatus.North);
    };
    const checkIfGameFinishedBecauseNoMoves = function () {
        const result = (position.moves.length === 0) ? true : false;
        if (result)
            reason = GameOverReason.NoMoves;
        return result;
    };
    const checkIfGameFinishedBecauseWinningScore = function () {
        ...
        if (result)
            reason = GameOverReason.PointsScored;
        return result;
    };
    const checkIfGameFinishedBecauseOfPlyCount = function () {
        const result = (position.ply - position.plyLastPoints >= position.plyTillEnd) ?
            true : false;
        if (result)
            reason = GameOverReason.PlyWithoutPoints;
        return result;
    };
    return (checkIfGameFinishedBecauseNoMoves() ||
        checkIfGameFinishedBecauseWinningScore() ||
        checkIfGameFinishedBecauseOfPlyCount()) ? [getResult(), reason, moves] :
