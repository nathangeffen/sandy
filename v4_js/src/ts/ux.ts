...
const createGameUX = function(divID: string, options: GameUXOptionType): GameUX {
  const div: HTMLDivElement | null = document.querySelector(`div#${divID}`);
  if (!div) {
    throw `Div element ${divID} does not exist.`;
  }
  return new GameUX(div, options);
}
...
setupBoard = function(this: GameUX, squareDim = 0) {
  ...
  if (!svg) return;
  const div: HTMLDivElement | null = this.div.querySelector('div.board-container');
  if (!div) return;
...
setupBoardEvents(this: GameUX) {
  const gameUX = this;
  const svg = this.get('board', 'svg') as SVGElement;
  if (!svg) return;
