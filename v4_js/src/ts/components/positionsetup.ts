...
export class PositionSetup {
  gameUX: GameUX;
  div: HTMLDivElement;
  action: string = "cursor";

  // Elements
  sideToPlay: HTMLInputElement | null;
  startScoreSouth: HTMLInputElement | null;
  startScoreNorth: HTMLInputElement | null;
  winScoreSouth: HTMLInputElement | null;
  winScoreNorth: HTMLInputElement | null;
  ply: HTMLInputElement | null;
  plyWhenLastPointsScored: HTMLInputElement | null;
  plyCountWithoutPointsTillEnd: HTMLInputElement | null;
  canMoveBackToLastMoveSquare: HTMLInputElement | null;
  ...
  setInitialSelectedToCursor = function(this: PositionSetup) {
    const pointer: HTMLButtonElement | null = this.div.querySelector('button.cursor');
    if (!pointer) return;
  ...
  addButtonEvents = function(this: PositionSetup) {
    const positionSetup = this;
    const buttons: NodeListOf<HTMLButtonElement> = this.div.querySelectorAll('div.setup-board-buttons button');
    const selected: HTMLDivElement | null = this.div.querySelector("div.selected");
    if (!selected) return;
  ...
  addFormEvents = function(this: PositionSetup) {
    const positionSetup = this;
  ...
  addEvents = function(this: PositionSetup) {
    this.addButtonEvents();
    this.addFormEvents();
  }

  update = function(this: PositionSetup) {
    const position = this.gameUX.game.position;
    this.gameUX.gameUXState = GameUXState.SettingUp;
